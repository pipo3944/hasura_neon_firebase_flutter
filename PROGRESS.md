...
## 📍 現在のフェーズ: ローカル環境構築 & スキーマ設計完了

次のステップ: **ローカル環境での動作確認・テスト**
...
- [ ] シードデータ作成
  - [ ] テスト用組織（1-2件）
  - [ ] テストユーザー（各ロール: user, tenant_admin, admin）
  - [ ] テスト投稿データ（10-20件）
- [ ] GraphQL API テスト
  - [ ] Admin Secret で基本クエリ実行
  - [ ] 各ロールでのパーミッション動作確認
  - [ ] リレーション（user, organization）の動作確認
- [ ] パーミッションテスト
  - [ ] user ロール: 自分のデータのみアクセス可能か
  - [ ] tenant_admin ロール: テナント内全データアクセス可能か
  - [ ] admin ロール: 全データアクセス可能か
  - [ ] 削除済みデータ（`deleted_at`）のフィルタ動作確認
...
- [x] **M1: ローカル環境構築** ← 完了！
- [ ] **M2: ローカル環境での動作確認** ← 次はここ
- [ ] **M3: Firebase Auth + Neon DB 設定**
- [ ] **M4: Dev環境デプロイ**
- [ ] **M5: Flutter アプリ初期実装**
- [ ] **M6: CI/CD構築**
- [ ] **M7: Prod環境デプロイ**
